if (typeof requirejs != 'undefined') {
	requirejs(['jquery'], initializeSchemakerView);
} else {
	initializeSchemakerView(jQuery.noConflict(TRUE));
}


function initializeSchemakerView(jQuery) {
	jQuery(document).ready(function($) {
		var viewingClass = $('#viewing-classname').val();
		var timer;
		jQuery('#extensionSelector, #versionSelector').change(function() {
		...
		var filter = jQuery('#viewhelper-filter').keyup(function() {
			clearInterval(timer);
			timer = setTimeout(search, 100);
		});
		if (typeof filter.tooltip == 'function') {
			filter.tooltip({
				'title': 'Tip: Use TAB for quick access',
				'trigger': 'hover'
			});
		}
		jQuery('.viewhelper-group').not('.root').each(function() {
			var element = $(this);
